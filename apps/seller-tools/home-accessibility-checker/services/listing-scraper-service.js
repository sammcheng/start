...
class ListingScraperService {
  ...
  async fetchListingHtml(url) {
    ...
    if (!response.ok) {
      const error = new Error(`Listing page fetch failed with status ${response.status}`);
  ...
  extractFromJson(value) {
    ...
    const typed = value;
    if (typed["@type"] === "House" || typed["@type"] === "SingleFamilyResidence" || typed.address) {
      ...
      return {
        ...
        bedrooms: this.readFact(typed, ["numberOfRooms", "numberOfBedrooms"]) || "N/A",
        bathrooms: this.readFact(typed, ["numberOfBathroomsTotal", "numberOfBathrooms"]) || "N/A",
